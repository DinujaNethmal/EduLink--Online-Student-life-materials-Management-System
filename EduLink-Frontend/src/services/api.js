// ============================================================
// services/api.js — Centralized API Client
// ============================================================
// Creates an Axios instance with base URL and auth interceptor.
// All API calls (auth, products, cart, orders, admin) are
// exported as named functions for use throughout the app.
//
// Guest users get a unique anonymous UUID (stored in localStorage)
// so each browser has its own cart and order history.
// ============================================================

// Add this export alongside your existing getOrders, etc.

/**
 * processPayment — POST /orders/:orderId/pay
 *
 * @param {string} orderId  - The MongoDB _id of the order
 * @param {object|FormData} paymentData
 *        For credit card: plain object { method, cardHolder, last4, brand }
 *        For bank transfer: FormData (includes paymentSlip file)
 */
export const processPayment = (orderId, paymentData) => {
  const isFormData = paymentData instanceof FormData;
  return axios.post(
    `/orders/${orderId}/pay`,
    paymentData,
    {
      headers: {
        ...authHeader(),
        // Let axios set Content-Type automatically for FormData (multipart/form-data)
        ...(isFormData ? {} : { "Content-Type": "application/json" }),
      },
    }
  );
};

import axios from "axios";

const API = axios.create({
  baseURL: "/api",
});

// Attach auth token to every request if available
API.interceptors.request.use((config) => {
  const token = localStorage.getItem("token");
  if (token) {
    config.headers.Authorization = `Bearer ${token}`;
  }
  return config;
});

// Helper: get current user ID from localStorage.
// Logged-in users → their real user ID.
// Guests → a unique anonymous UUID generated once and persisted in localStorage,
// so each browser gets its own cart and order history.
const getUserId = () => {
  const user = localStorage.getItem("user");
  if (user) return JSON.parse(user).id;

  // Guest: reuse or generate a unique anonymous ID
  let guestId = localStorage.getItem("guestId");
  if (!guestId) {
    guestId = "guest-" + crypto.randomUUID();
    localStorage.setItem("guestId", guestId);
  }
  return guestId;
};

// ==================== AUTH APIs ====================

export const registerUser = (data) => API.post("/auth/register", data);
export const loginUser = (data) => API.post("/auth/login", data);
export const getMe = () => API.get("/auth/me");
export const deleteAccount = () => API.delete("/auth/me");

// ==================== PRODUCT APIs ====================

export const getProducts = (params) => API.get("/products", { params });
export const getProductById = (id) => API.get(`/products/${id}`);
export const createProduct = (data) => API.post("/products", data);
export const updateProduct = (id, data) => API.put(`/products/${id}`, data);
export const deleteProduct = (id) => API.delete(`/products/${id}`);

// ==================== CART APIs ====================

export const getCart = () => API.get(`/cart/${getUserId()}`);
export const addToCart = (productId, quantity = 1) =>
  API.post("/cart/add", { userId: getUserId(), productId, quantity });
export const updateCartItem = (productId, quantity) =>
  API.put("/cart/update", { userId: getUserId(), productId, quantity });
export const removeFromCart = (productId) =>
  API.delete(`/cart/${getUserId()}/item/${productId}`);
export const clearCart = () => API.delete(`/cart/${getUserId()}`);

// ==================== ORDER APIs ====================

export const buyNow = (productId, quantity = 1) =>
  API.post("/orders/buy-now", { userId: getUserId(), productId, quantity });
export const checkout = () =>
  API.post("/orders/checkout", { userId: getUserId() });
export const getOrders = () => API.get(`/orders/${getUserId()}`);
export const getOrderById = (orderId) => API.get(`/orders/detail/${orderId}`);

// ==================== ADMIN APIs ====================

export const getAdminStats = () => API.get("/admin/stats");
export const getAdminUsers = () => API.get("/admin/users");
export const adminDeleteUser = (id) => API.delete(`/admin/users/${id}`);
export const adminUpdateUserRole = (id, role) => API.put(`/admin/users/${id}/role`, { role });
export const getAdminProducts = () => API.get("/admin/products");
export const adminDeleteProduct = (id) => API.delete(`/admin/products/${id}`);
