...
exports.createGroupBanner = async (req, res) => {
  try {
    const bannerData = {
      ...req.body,
      leaderName: req.body.leaderName || req.body.groupName || "Group Leader",
      wantedGender: req.body.wantedGender || "Any",
    };
    const banner = await GroupBanner.create(bannerData);
...
exports.updateGroupBanner = async (req, res) => {
  try {
    ...
    const banner = await GroupBanner.findByIdAndUpdate(req.params.id, payload, { new: true, runValidators: true });
    if (!banner) {
